...
export default function (RequestList, callback) {
    var approvalRequestList = ApiInfo.baseUrl+"rejectTimesheetEntry?emailAddress="+RequestList.MailID;
    ...
    var postData = {
        ...
        "REJECT_REASON" : RequestList.reason
     };

    Fetch.post(approvalRequestList,postData, function (response) {
        if (response.success) {
            console.log("TimeSheet Reject data",response)
            if (response.SUCCESS) {
                callback({
                    success: true,
