...
import { StyleSheet, StatusBar, AsyncStorage, Dimensions, BackHandler, TouchableOpacity, Alert, 
    Platform,View,ImageBackground,Image,NativeModules,UIManager,LayoutAnimation,Modal,
 } from 'react-native';
                                    ...
                                    <View style={{width:'10%',justifyContent:'flex-start'}}>
                                        { !value.IsSelect 
                                            ...
                                            <View style={{ height:65, borderRadius:10,backgroundColor:'#59b7d3',padding:5, borderWidth:2, marginTop:-35,borderColor:'transparent'}}>
                                                <Text style={{color:'#fff',fontWeight:'600',textAlign:'center',fontSize:10}}>{value.REMAINING_HOURS} </Text>
                                            </View>
                                            ...
                                            <View style={{ height:65, borderRadius:10,backgroundColor:'#006fb1',padding:5, borderWidth:2, marginTop:-35,borderColor:'transparent'}}>
                                                <Text style={{color:'#fff',fontWeight:'bold',textAlign:'center',fontSize:10}}>{value.REMAINING_HOURS} Hrs</Text>
                                            </View>
                            ...
                            <View padder style={{flexDirection: 'row',flex:1, width: SCREEN_WIDTH}}>
                                ...
                                <View style={{width:'10%',justifyContent:'flex-start'}}>
                                    { !value.IsSelect 
                                        ?
                                        <View style={{ height:65, borderRadius:10,backgroundColor:'#59b7d3',padding:5, borderWidth:2, marginTop:-35,borderColor:'transparent'}}>
                                            <Text style={{color:'#fff',fontWeight:'600',textAlign:'center',fontSize:10}}>{value.DURATION} </Text>
                                        </View>
                                            // <Icon type="AntDesign" name="checkcircle" style={{ fontSize: 16, color: '#c0c0c0' }} />
                                        : 
                                        <View style={{ height:65, borderRadius:10,backgroundColor:'#006fb1',padding:5, borderWidth:2, marginTop:-35,borderColor:'transparent'}}>
                                            <Text style={{color:'#fff',fontWeight:'bold',textAlign:'center',fontSize:10}}>{value.DURATION} Hrs</Text>
                                        </View>
                                            // <Icon type="AntDesign" name="checkcircle" style={{ fontSize: 16, color: '#7666fe' }} />
                                    }    
                                </View>
...
class Home extends React.Component {
    ...
    approveService =(item,index) => {
        ...
        console.log(" Item to Approve",scheduleItems,index)
        // for(let i = 0;i<item.length ; i++){
            ...
            // console.log("Emp name testing",empName, "then",payload)
            const arr = [...this.state.scheduleItems];
            arr.splice(index, 1);
            this.setState({
                scheduleItems : arr,
                listType : false
            ...
            ()=>{
                console.log("Items after Approved",this.state.scheduleItems)
            })
                ...
                if (response.success) {
                    if (response.data.length > 0) {
                        // if(item.length == 1 ){
                            ...
                            var updatedQuotes =  scheduleItems.filter((row, index) => {
                                return row['TIMESHEET_LINE_ID'] != item.TIMESHEET_LINE_ID;
                            });
                            this.setState({
    ...
    showrejectionModal(item,index,Isvisible){
        console.log("Item to rejection",this.state.scheduleItems[index],);
        this.setState({
            ...
            <Container >
                 <ImageBackground source={require('../../assets/background.png')} style={styles.background}> 
                    <Header  style={{ backgroundColor: '#580073' }}>
                        <Left>
                            ...
                            > */}
                                <Thumbnail  source={require('../../assets/Icon-1024.png')} style={{  height: 30, width: 40 }} />
                            {/* </TouchableOpacity> */}
                        </Left>
                        <Body>
                ...
                <View  style={{flexDirection: 'row',flex:1, width: SCREEN_WIDTH,padding:5,paddingLeft:20}}>
                    ...
                    <TouchableOpacity onPress ={()=> this.changeListType(false)}>
                        {/* <Thumbnail circular source={require('../../assets/casual.png')} /> */}
                        <View style = { !this.state.listType ? styles.selectedcircle : styles.unselectedcircle}>
